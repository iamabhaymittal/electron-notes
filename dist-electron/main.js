...
const getNotes = async () => {
  ...
  await lib.ensureDir(rootDir);
  const notesFileNames = await fs$i.readdir(rootDir, {
...
const getNoteInfoFromFileName = async (fileName) => {
  const fileStat = await lib.stat(`${getRootDir()}/${fileName}`);
  return {
...
const readNote = async (fileName) => {
  const rootDir = getRootDir();
  return lib.readFile(`${rootDir}/${fileName}.md`, {
    encoding: fileEncoding
  });
};
const writeNote = async (fileName, content) => {
  ...
  return lib.writeFile(`${rootDir}/${fileName}.md`, content, {
    encoding: fileEncoding
  });
};
...
function createWindow() {
  const iconPath = process.env.VITE_PUBLIC ? path$c.join(process.env.VITE_PUBLIC, "electron-vite.svg") : "";
  win = new BrowserWindow({
    ...
    frame: false,
    // Enable the frame to allow window dragging
...
app.whenReady().then(() => {
  ...
  ipcMain.handle(
    "readNote",
    (_, ...args) => readNote(...args)
  );
  ipcMain.handle(
    "writeNote",
    (_, ...args) => writeNote(...args)
  );
  createWindow();
});
ipcMain.on("ping", () => {
